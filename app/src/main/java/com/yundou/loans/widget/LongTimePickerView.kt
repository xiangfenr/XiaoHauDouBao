...
/**
 *@ClassName LongTimePickerView
 *@Deseription TODO
 *@author：wangmingyu
 *@date：2021/3/2020:50
 */
class LongTimePickerView : BasePickerView, View.OnClickListener {
    ...
    private val TAG_LONG_VALID = "long_valid"
    private var isLongTime=true
    constructor(pickerOptions: PickerOptions,isLongTime:Boolean) : super(pickerOptions.context) {
    ...
    private fun initView(context: Context) {
        ...
        if (mPickerOptions.customListener == null) {
            LayoutInflater.from(context).inflate(R.layout.pickerview_time_long, contentContainer)
            
            //顶部标题
            val tvTitle = findViewById(R.id.tvTitle) as TextView
            val rv_top_bar = findViewById(R.id.rv_topbar) as RelativeLayout
            val long_valid = findViewById(R.id.tv_long_valid) as TextView
            if(!mPickerOptions.type[0]||!isLongTime)
                long_valid.visibility = View.GONE
            //确定和取消按钮
            ...
            long_valid.tag = TAG_LONG_VALID
            btnSubmit.setOnClickListener(this)
            btnCancel.setOnClickListener(this)
            long_valid.setOnClickListener(this)
...
//            setRangDate()
            ...
            require(mPickerOptions.endDate[Calendar.YEAR] <= (Calendar.getInstance().get(Calendar.YEAR)+100)) {
                "The " +
                    "endDate should not" +
                    " be " +
                    "later than ${(Calendar.getInstance().get(Calendar.YEAR)+100)}" }
//            setRangDate()
        }
        setRangDate()
        setTime()
        wheelTime!!.setLabels(mPickerOptions.label_year, mPickerOptions.label_month, mPickerOptions.label_day, mPickerOptions.label_hours, mPickerOptions.label_minutes, mPickerOptions.label_seconds)
        wheelTime!!.setTextXOffset(mPickerOptions.x_offset_year, mPickerOptions.x_offset_month, mPickerOptions.x_offset_day, mPickerOptions.x_offset_hours, mPickerOptions.x_offset_minutes, mPickerOptions.x_offset_seconds)
    ...
    private fun initDefaultSelectedDate() {
        //如果手动设置了时间范围
        if (mPickerOptions.startDate != null && mPickerOptions.endDate != null&&mPickerOptions
                .date==null) {
        ...
        } else if (mPickerOptions.startDate != null&&mPickerOptions.date==null) {
            //没有设置默认选中时间,那就拿开始时间当默认时间
            mPickerOptions.date = mPickerOptions.startDate
        } else if (mPickerOptions.endDate != null&&mPickerOptions.date==null) {
    ...
    override fun onClick(v: View) {
        ...
        } else if (tag == TAG_LONG_VALID) {
            val dateFormat: DateFormat = SimpleDateFormat("yyyy-MM-dd")
            val date=dateFormat.parse("9999-12-31")
